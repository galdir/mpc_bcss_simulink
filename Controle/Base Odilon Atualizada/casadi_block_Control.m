 ...
 %% ================     Inicialização dos parâmetros - só passa aqui uma única vez (equivale a Flag=0 na SFunction)
       function setupImpl(obj,~)
           ...
           import casadi.*                                    % Importação da biblioteca Casadi (tem de estar no path)
            ...
            R = evalin('base','R');                          % Peso das variações de controle nas entradas (Freq. e PMonAlvo)
            ...
            nx=height(XIni);                                      % Numero de variáveis (estados) do processo
            UIni=evalin('base','UIni');                       % Condições iniciais das entradas (variáveis manipuladas) do processo
            nu=height(UIni);                                      % Numero de variáveis de entrada no processo
            ...
            ny=height(YIni);                                      % Numero de variáveis de saida (controladas por setpoint)
...
%                 if t>500 && ~Feasible     % Não estava sendo tratado, mas se tratar, engasga na inicalização
%                                                          % Esta não pode ser a solução. Aleatoriedade... note que dá problema no PlanoAceleracaoErro!!
%                     DeltaU=[0;0];               % Assume variação nula,  mas não na inicialização. 
%                 end
