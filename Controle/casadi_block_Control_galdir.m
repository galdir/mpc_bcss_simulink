...
classdef casadi_block_Control< matlab.System & matlab.system.mixin.Propagates
    ...
    methods (Access = protected)
        ...
        function  SaidaMPC= stepImpl(obj,X_atual,U_atual,AlvoEng,t)
            ...
            ErroX=X_atual-XPredito;                            % Erro entre as medições atuais e a predição feita no instante anterior

            Y0=full(Funcao_h(X_atual));                                      % Saidas controladas por setpoint
            ...
            obj.x0=[X_atual; obj.x0(1:end-nx)];  % Atualiza condição inicial dos estados com a medição atual e valores passados
            obj.u0=[U_atual; obj.u0(1:end-nu)];  % Atualiza condição inicial das açoes de controle entrada atual e valores passados
            ...
            if (obj.contador >= PassoMPC)    % Solver só entra no passo definido pelos parâmetros de Passo do MPC ou se deu Unfeasible na amostra anteriior
                ...
                args.p=[    X_atual;          U_atual;     AlvoEng;    Ysp;    ErroX;     ErroY;   obj.BuffDeltaFreq;     obj.ModeloPreditor.data.a0];
...
%                     end
            ...
            [x_predito, ESNdataa0] = executa_predicao(U_atual,X_atual, ModeloPreditor.data.a0, ModeloPreditor, obj.EstimaVazao);
