    ...
    methods (Access = protected)
        ...
        function setupImpl(obj,~)
            ...
            for k=1:Hp
                ...
                LimMaxX=LimitesX(:,1)-X(:,k);    % Para não ser violado o limite, a diferença deve ser >= 0
                g=[g; LimMaxX];
                ...
                LimMinX=X(:,k)-LimitesX(:,2);      % Para não ser violado o limite, a diferença deve ser >= 0
                g=[g; LimMinX];
                args.lbg=[args.lbg,   zeros(1,nx) ];    % Limite mínimo para restrição de desigualdade
                args.ubg=[args.ubg,      inf(1,nx) ];    % Limite máximo para restrição de desigualdade
                ...
                y_saida= h(X(:,k));                       % Saidas preditas (variáveis controladas por setpoint)
            ...
            for k=1:Hp                                                    % Para todo o horizonte de predição
                ...
                %y_saida= h(X(:,k+1));                             % Saida estimada (variáveis controladas por setpoint - retorna coluna) (Galdir: pq k+1? e se k+1, o que ocorre acima de hp?)
                y_saida= h(X(:,k));                             % Saida estimada (variáveis controladas por setpoint - retorna coluna)
