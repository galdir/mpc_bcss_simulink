...
classdef casadi_block_Control < matlab.System & matlab.system.mixin.Propagates
    ...
    properties (Access = private)                       % Criação das variáveis que vão compor o OBJETO
        ...
        BufferDeltaU                                               % Para guardar as últimas 14 variações na ação de controle
...
%        TipoPreditor=ModeloPreditor.data.tipo;                    % Verifica tipo 1=ESN, 2 = LSTM (USAMOS APENAS ESN)
           ...
           obj.ysp0=y0;                                          % Inicializa Ysp com os valores atuais na saida da planta
           du0=zeros(Hc,nu);                                 % Estrutura para armazenar DeltaU atuais e futuros, até o horizonte Hc 
           obj.du0=du0(:);                                       % Condição inicial para os DeltaU em todo o horizonte Hc futuro
           obj.BufferDeltaU=zeros(14,1);              % Inicializa Buffer com zeros
...
%             DU = MX.sym('DU',Hc,nu);                      % Incrementos do controle sobre o horizonte Hc (Variável de decisão)
            ...
            Indice=[  nx                        nu            nu*Hc          nu            nx             ny         14       nx_ESN];
            ...
            [ Xk0,Uk0, DU0,AlvoEng,ErroX, ErroY, BufferDeltaU, Reservatorio_ESN]=ExtraiParametros(P,Indice);
           ...
           %% Montar agora as restrições de igualdade/desigualdade em g
            ...
            for k=1:Hp
               ...
               % o horizonte de predição. Tudo isso ponderado pela matriz Qy
                ...
                Uk0=Uk0+ DU(k,:);
                BufferDeltaU=ShiftDown(BufferDeltaU,DU(k,1));    % Apenas o Delta da Frequencia
                g=[g; sum(BufferDeltaU)];         % Insere restrição de desigualdade para o somatório do BufferDeltaU
...
%             disp(strcat("Simulação MPC em ",num2str(t)," s"))
            ...
            DeltaU=zeros(1,nu);                     % Reinicia DeltaU=0, inclusive para saber quando não passou pelo Solver
            ...
            if (obj.contador==PassoMPC)      % Solver só entra no passo definido pelos parâmetros de Passo do MPC
               ...
               %           [        Medidas       u0;       du0         AlvoEng;   Err_Xpredito;   Err_YPredito;      BufferDeltaU      Reservatório do ModeloPreditor]
                args.p=[DadosProcesso;  u0;    obj.du0      AlvoEng;      ErroX;               ErroY;           obj.BufferDeltaU;         obj.ModeloPreditor.data.a0];     
                ...
                args.x0=[ x0;  du0;  ysp0];               % Atualizada condição incial das variáveis de decisão com base nos valores antes praticados
               ...
               %% Resgata estruturas de restrições guardadas pelo objeto
                ...
                x0=Solucao_MPC(1:nx*(1+Hp));                % Assume nova condição inicial x0 para os estados atuais e preditos 
                du0=Solucao_MPC(nx*(1+Hp)+1:nx*(1+Hp)+nu*Hc); % Resgata novas ações de controle (Delta U) ótimos calculadas em todo o horizonte Hc
                DeltaU=du0(nu,1);                                           % Extrai DeltaU ótimo à ser aplicado (primeiro da fila)
                ysp0=Solucao_MPC(end-ny+1:end);            % Extrai Ysp ótimos calculados (últimas ny colunas do vetor)
                
                % Atualiza objetos da função para o próximo ciclo (x0 deve ser atualizado fora do loop pois nem sempre passará pelo Solver)
                obj.du0=du0;
                obj.ysp0=ysp0;
                obj.BufferDeltaU=ShiftDown(BufferDeltaU,DeltaU);
            ...
            SaidaMPC=[Feasible; Iteracoes; TempoSolver; u0(:); DeltaU(:); ysp0(:)];
            
            % Passando ou não pelo Solver precisa atualizar os estados do processo e o reservatório da ESN
            obj.x0=x0;
...
function   [ Xk0,Uk0, DU0,AlvoEng,ErroX, ErroY, BufferDeltaU, Reservatorio_ESN]=ExtraiParametros(P,Indice);
