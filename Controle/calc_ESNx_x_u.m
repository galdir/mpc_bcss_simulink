function [sys,x0] = calc_ESNx_x_u(t,x,u,flag,ESN,YIni,MatrizSimulador,UIni)
...
if flag == 0
	...
	nout=nx+1+nu;			           % Número de variáveis do processo + tempo de processamento		 
    x0 = [ YIni; ESN.a0; 0; UIni];  % Condição inicial das variáveis do processo, do reservatório da ESN e do tempo de processamento
    sys= [0;size(x0,1);nout;ninput;0;0];  
    %sys= [nx;1;size(ESN.a0,1);1;1;1];  
...
elseif abs(flag) == 2
    ...
    xres = x(nx+1:end-1-nu);                                    % Resgata estados do reservatório 
    ...
    sys =[yk_1;VazaoEstimada; xres;t_execucao;u(1);u(2)];	   % Retorna o vetor de estados em coluna

% =========================================================================
% flag = 3 --> Retorna vetor de saida
% =========================================================================
elseif flag == 3
	sys=[x(1:nx) ; x(end-2);x(end-1); x(end)];   % Retorna apenas as novas variáveis estimadas e o tempo de execução  
end
% =========================================================================
