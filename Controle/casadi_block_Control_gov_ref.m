...
classdef casadi_block_Control_gov_ref< matlab.System & matlab.system.mixin.Propagates
    ...
    methods (Access = protected)
        ...
        function sz1 = getOutputSizeImpl(obj, ~)         % Organização da saida do bloco Casadi
            ...
            Dim =   [1 ... % viabilidade (se viavel/feasible)
                ...
                nx*(1+Hp) ... % Xk
                nu*Hp ...
                ny ... % desvio de sp do gov ref
                nu ... % desvio de manipualdas do gov ref
                ]; % Uk
        ...
        function setupImpl(obj,~)
            ...
            [solver, args_solver] = cria_solver_gov_ref(obj.umax, obj.umin, obj.dumax, obj.MargemPercentual, ...
        ...
        function  SaidaMPC= stepImpl(obj,X0,U0,AlvoEng,t)
            ...
            delta_ysp0 = zeros(ny, 1);
            delta_alvoeng0 = zeros(nu, 1);
            ...
            if (obj.contador >= PassoMPC)    % Solver só entra no passo definido pelos parâmetros de Passo do MPC ou se deu Unfeasible na amostra anteriior
                ...
                if Feasible
                    ...
                    [Xk,Uk,DeltaUk, delta_ysp0, delta_alvoeng0]=ExtraiSolucao(Solucao_MPC,Indice);
            ...
            SaidaMPC    = [Feasible; Iteracoes; TempoSolver; U0; DeltaU;  SomaDeltaFreq; Jy; Ju; Jr; Jx;  ErroX;  ErroY;  Ysp;        Xk;               Uk; delta_ysp0; delta_alvoeng0    ];
...
function   [Xk,Uk,DeltaUk, delta_ysp, delta_u]=ExtraiSolucao(Solucao,Indice)
    Ate=[Indice(1)   sum(Indice(1:2))  sum(Indice(1:3)) sum(Indice(1:4)) sum(Indice(1:5))  ];
    De=[1  Ate(1)+1   Ate(2)+1  Ate(3)+1    Ate(4)+1 ];
    Xk=Solucao(De(1):Ate(1));
    Uk=Solucao(De(2):Ate(2));
    DeltaUk=Solucao(De(3):Ate(3));
    delta_ysp=Solucao(De(4):Ate(4));
    delta_u=Solucao(De(5):Ate(5));
end
