...
% 
% Usada para plotar resultados do exemplos 7x
close all
figure(1)
set(gcf,'position',[120  400  560  420]);
subplot(2,1,1)
plot(u_cl(:,1),'bx')
subtitle('Velocidade linear (v)')
ylabel('Velocidade linear (v)')
grid on
subplot(2,1,2)
plot(u_cl(:,2),'bx')
subtitle('Velocidade angular (omega)')
...
% Deleciona área para o plot de acordo com os limites de posição definidos para o robô no respectivo exemplo
axis([ FixedStatesLowerBounds(1)   FixedStatesUpperBounds(1) FixedStatesLowerBounds(2)   FixedStatesUpperBounds(2)])
hold on
grid on
...
% Tamaho do raio do robô default para o plot de inicialização. Pode de ajustado para restrição dinâmica
RaioRobo=0.3;                                                 
...
function  plotagens=plot_vetores(M,Hp,A,R,Obstaculo)
    ...
    % - A linha 1 representa o valor dos estados no instante k atual
