...
plotagens=plot_vetores(xx1(:,:,1),N,A,R,'b');    % Inicializa vetores com dados do plot para a animação
for k=2:size(xx1,3)
    ...
    for i=1:N+1
        ...
        R=RaioProtecaoDinamica(px,py);     % Tamanho do raio de proteção em função das coordenadas
        [nx  ny]=Pontos_circulo(px,py,R);
...
function  plotagens=plot_vetores(M,Hp,A,R,cor);
    ...
    for i=1:Hp+ 1
        ...
        R=RaioProtecaoDinamica(px,py);    % Extrai o tamanho do circulo de proteção em função das coordenadas
