...
FixedStatesLowerBounds= [-2 -2   -inf];       % Xrobo >- 2    Yrobo > -2     Orientação Teta  > -inf
FixedStatesUpperBounds= [ 2   2   inf];         % Xrobo < 2     Yrobo < 2      Orientação Teta < inf
...
P=SX.sym('P',n_states+n_states);           % Parametros que vão  conter os estados em k (1:3) e os estados no k+1 (4:6). Definidos em args.p
...
% este mesmo formato para os valores futuros preditos
args.lbx=[];       % Inicializa limites inferiores para as restrições dos estados X e ações de controle
args.ubx=[];     % Inicializa limites superiores para as restrições dos estados X e ações de controle

% Inicializa variável para guardar as fórmulas das restrições que vamos criar livremente
% Neste caso, o formato que for criado também deve ser considerado para os valores no horizonte de predição
g=[];             
...
st=X(:,1);                     % Guarda variável com estados atuais X0 do sistema (sendo X uma variável simbólica)
...
g=[g;st-P(1:n_states)];   % Restrição como diferença entre estados. Fará a conta entre os estados atuais e os estados desejados (xs)    
% Sendo a diferença entre os estados atuais e desejados é uma restrição dita como sendo uma restrição de igualdade
...
% Nas últimas linhas anteriores criamos os valores para os estados d/atuais o sistema (x,y,Teta). 
...
for k=1:N                       % Insere restrições para todo o horizonte de predição (contas feitas em variável simbólica para passar ao Solver)
    ...
    con=U(:,k);                 % Ações de controle atual (v omega)
...
%    obj=obj + (st-P(4:6))'*Q*(st-P(4:6)) + con'*R*con;    % Função objetivo do MPC que deve ser minimizada
    ...
    st_next=X(:,k+1);       % Olha o passo seguinte 
    f_value=f(st,con);      % Aplica a função dinâmica para o passo seguinte
    st_next_euler=st+(T*f_value);   % Estima um valor futuro com base no método de Euler
%     st_next_euler=st_next_euler+randn/100;   % Estima um valor futuro com base no método de Euler
...
for k=1:N+1     
    ...
    DistanciaMinEntreCentros= robo_r+obs_r;             % Distancia minima necessária entre os centros para não haver colisão
    DistanciaAtual=sqrt((Posicao(1)-Obstacle(1))^2 + (Posicao(2)-Obstacle(2))^2);   % Distância entre os centros
    Diferenca=DistanciaAtual-DistanciaMinEntreCentros; % Diferença em relação a distância minima      
...
% Saida so Solver. Dimensão = [ EstadosAtuais + EstadosFuturos em todo HP  +  Ações de controle em todo Hp ]
...
disp('Montado o solver para o Exemplo 71')
...
while (norm((x0-xs),2) > 1e-2 && mpciter<sim_time/T)
    ...
    Solucao=full(sol.x);    % Saida so Solver. Dimensão = [ EstadosAtuais + EstadosFuturos em todo HP  +  Ações de controle em todo Hp ]
