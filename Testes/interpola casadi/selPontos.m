function Pontos=selPontos(Freq,Press,T)
    ...
    if Freq<min(gridF) |  Freq>max(gridF)     % Avalia se o ponto fornecido para interpolar está dentro da faixa
        ...
        if Freq<min(gridF) 
            Freq=min(gridF);   
        end      % Se valor passado está abaixo da faixa, assume o valor mínimo do grid definido     
        if Freq>max(gridF) 
            Freq=max(gridF); 
        end     % Se valor passado está acima da faixa, assume o valor máximo do grid definido
